...
// Verificar si Amplify ya está configurado para evitar reconfiguración
let isAmplifyConfigured = false;

try {
  // Intentar acceder a la configuración actual
  const currentConfig = Amplify.getConfig();
  if (currentConfig && currentConfig.Auth && currentConfig.Auth.Cognito) {
    isAmplifyConfigured = true;
    console.log('✅ Amplify ya está configurado');
  }
} catch (error) {
  // Si falla, significa que no está configurado
...
if (!isAmplifyConfigured) {
  ...
  console.log('ℹ️ Amplify ya estaba configurado, omitiendo reconfiguración');
}