...
const PAYMENT_METHODS: MethodOption[] = [
  {
    ...
    icon: (
      <div className="flex gap-2">
        ...
        <svg viewBox="0 0 24 16" className="w-8 h-8" xmlns="http://www.w3.org/2000/svg">
  ...
  },
  {
    id: 'invoice',
    icon: (
      <svg viewBox="0 0 24 24" className="w-8 h-8 text-indigo-600" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" xmlns="http://www.w3.org/2000/svg">
          ...
          >
            {isSelected && (
              <div className="absolute top-0 right-0 w-12 h-12 overflow-hidden rounded-tr-2xl">
                <div className="absolute -top-6 -right-6 w-12 h-12 bg-indigo-600 rotate-45" />
                <svg className="absolute top-1.5 right-1.5 w-3.5 h-3.5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
            ...
            <div className="flex items-center gap-6">
              <div className="flex-shrink-0 flex items-center justify-center w-16 h-12 bg-white rounded-lg border border-gray-100 shadow-sm">
                {method.icon}
              </div>

              <div className="flex-1">
                <h4 className={`text-lg font-bold \${isSelected ? 'text-indigo-900' : 'text-gray-900'}`}>
                  {method.title}
                </h4>
                ...
                {method.badges && (
                  <div className="flex gap-2 mt-3">
                    {method.badges.map((badge) => (
                      <span key={badge} className="px-2.5 py-1 text-xs font-bold bg-white text-indigo-700 border border-indigo-100 rounded-md">
                        {badge}
                      </span>
                    ))}
                  </div>
