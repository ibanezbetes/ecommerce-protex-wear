...
import { sendOrderConfirmationEmails, generateOrderNumber, OrderEmailData } from '../services/emailService';
...
const STEPS = [
  { number: 1, title: 'Dirección', icon: '📍' },
  { number: 2, title: 'Envío', icon: '🚚' },
  { number: 3, title: 'Pago', icon: '💳' },
  { number: 4, title: 'Confirmar', icon: '✅' },
];
...
function CheckoutPage() {
  ...
  useEffect(() => {
    if (items.length === 0 && !orderPlaced) navigate('/carrito');
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      if (paymentMethod === 'card') {
        // Stripe flow handled by CheckoutButton
        return;
      }

      // For bank_transfer and bizum, mark as placed
      setOrderPlaced(true);
      clearCart();
      navigate('/success', {
        state: {
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, color: 'rgba(255,255,255,0.7)', fontSize: '0.8125rem' }}>
                Proceso seguro con encriptación SSL
              </p>
            </div>
          </div>
          <button
            ...
            {currentStep === 1 && (
              <div className="checkout-card animate-fade-in-up">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  {[
                    { field: 'firstName' as keyof Address, label: 'Nombre *', placeholder: 'María', cols: 1 },
                    { field: 'lastName' as keyof Address, label: 'Apellidos *', placeholder: 'García López', cols: 1 },
                    { field: 'company' as keyof Address, label: 'Empresa (opcional)', placeholder: 'Empresa S.L.', cols: 2 },
                    { field: 'addressLine1' as keyof Address, label: 'Dirección *', placeholder: 'Calle Mayor, 15, 2ºB', cols: 2 },
                    ...
                    { field: 'postalCode' as keyof Address, label: 'Código Postal *', placeholder: '28001', cols: 1 },
                    { field: 'phone' as keyof Address, label: 'Teléfono', placeholder: '+34 600 000 000', cols: 2 },
                  ...
                  <div style={{
                    ...
                    <div>
                      ...
                      <p style={{ margin: 0, fontSize: '0.8125rem', color: '#6b7280' }}>
                        Procesarás el pago en el siguiente paso. Aceptamos Visa, Mastercard y Amex.
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="checkout-card animate-fade-in-up">
                ...
                <div style={{ marginBottom: '1.25rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.625rem' }}>
                    <h4 style={{ margin: 0, fontSize: '0.875rem', fontWeight: 700, color: '#374151' }}>📍 Dirección de envío</h4>
                ...
                <div style={{ marginBottom: '1.25rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.625rem' }}>
                    <h4 style={{ margin: 0, fontSize: '0.875rem', fontWeight: 700, color: '#374151' }}>🚚 Método de envío</h4>
                  ...
                  <div className="review-address-card">
                    <p style={{ margin: 0, fontSize: '0.875rem', color: '#374151' }}>
                      {shippingOption?.description} — <span style={{ color: '#10b981', fontWeight: 600 }}>Llega el {getDeliveryDate(shippingOption?.estimatedDays || 4)}</span>
                    </p>
                  </div>
                </div>
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.625rem' }}>
                    <h4 style={{ margin: 0, fontSize: '0.875rem', fontWeight: 700, color: '#374151' }}>💳 Método de pago</h4>
                  ...
                  <div className="review-address-card">
                    <p style={{ margin: 0, fontSize: '0.875rem', color: '#374151' }}>
                      {paymentMethod === 'card' && '💳 Tarjeta de Crédito/Débito (Stripe)'}
                ...
                <label style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', cursor: 'pointer', marginBottom: '1.5rem' }}>
                  ...
                  <span style={{ fontSize: '0.875rem', color: '#6b7280', lineHeight: 1.5 }}>
                    ...
                    <a href="#" style={{ color: '#2e559e', fontWeight: 600 }}>Términos y Condiciones</a>{' '}
                    y la{' '}
                    <a href="#" style={{ color: '#2e559e', fontWeight: 600 }}>Política de Privacidad</a>.
                  </span>
                </label>
                ...
                {paymentMethod === 'card' ? (
                  acceptedTerms ? (
                    ...
                    <button disabled className="checkout-confirm-btn" style={{ opacity: 0.5, cursor: 'not-allowed' }}>
              ...
              <div style={{ padding: '1.25rem 1.5rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', paddingTop: '1rem', borderTop: '1.5px solid #f3f4f6' }}>
                  {[
                    ...
                    ...(discountCode ? [{ label: `Descuento (${discountCode})`, value: `-€${(discountAmount || 0).toFixed(2)}`, green: true }] : []),
                    { label: 'IVA (21%)', value: `€${tax.toFixed(2)}` },
                    { label: 'Envío', value: shippingCost === 0 ? '¡Gratis!' : `€${shippingCost.toFixed(2)}`, green: shippingCost === 0 },
                    ...
                    <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: '0.875rem', color: '#6b7280', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                        ...
                        {label.startsWith('Descuento') && (
                          <button type="button" onClick={() => { removeDiscountCode(); showToast('Descuento eliminado', 'info'); }} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', padding: 0, fontSize: '0.75rem' }}>✕</button>
                        )}
                      </span>
                ...
                <div style={{
                  ...
                  <p style={{ margin: 0, fontSize: '0.75rem', color: '#6b7280' }}>
                    🔒 Transacción segura · Datos cifrados
                  </p>
                </div>
              </div>
            </div>
